...
async function loadSwiper() {
  ...
  const { default: Swiper } =
    await import("../../scripts/swiper/swiper-bundle.min.js");
  return Swiper;
}
...
export default async function decorateSeeyourWealthInmotion(doc) {
  ...
  for (const section of sections) {
    ...
    fraction.innerHTML =
      '<span class="sywim-fraction-current">1</span>/<span class="sywim-fraction-total">' +
      totalSlides +
      "</span>";
    ...
    const swiper = new Swiper(swiperContainer, {
      ...
      on: {
        slideChange(s) {
          fraction.querySelector(".sywim-fraction-current").textContent =
            s.activeIndex + 1;
        },
      },
    });
